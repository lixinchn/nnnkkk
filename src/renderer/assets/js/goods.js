...
export default{
	...
	Buildorder(info,cb){
		...
		fun.getSign(info.cookie,info.itemid,info.sku,(error, res)=>{
			var url = `https://h5acs.m.tmall.com/h5/mtop.trade.buildorder.h5/3.0/?jsv=2.4.7&appKey=12574478&t=${res.time}&sign=${res.sign}&api=mtop.trade.buildOrder.h5&v=3.0&type=originaljson&timeout=20000&isSec=1&dataType=json&ecode=1&ttid=%23b%23ip%23%23_h5&AntiFlood=true&LoginRequest=true&H5Request=true&x-itemid=${info.itemid}&x-uid=${info['x-uid']}`
			su.post(url)
				...
				.end((error, result)=>{
					if(result.headers['set-cookie']){
						...
						setTimeout(() => {this.createOrder(info,JSON.parse(result.text),cb)}, 0);
					}
					cb(error,null)
					return
				})
	...
	createOrder(info,realPay,cb){
		// if (realPay.data.url) {
		// 	location.href = realPay.data.url;
		// 	return;
		// }
		//创建订单下单
		console.log(realPay);
